...
(async () => {
  ...
  for (const file of ['auth.js','supabase.js','navbar.js','clinical-open.js','patient-lifecycle.js',
                      'ask.html','questions.html','patients.html'])
    t('untouched: ' + file, read(file) === onMain(file), file);
  /* dashboard.html is asserted differently, and ONLY dashboard.html. Byte
     equality against main is a branch-scope guard rather than a property of
     the founder page: it fails the moment any later branch edits the file for
     a reason that has nothing to do with founders, which is what happens on
     the branch that reworks the New Patient dialog. The claim this section
     actually makes about dashboard.html — that the founder page did not spill
     into it — is asserted directly instead. The other eight files keep byte
     equality, because for them the freeze IS the claim. */
  t('no founder content leaked into dashboard.html',
    !/founder\.html|founder-giga|Meet the founder/i.test(read('dashboard.html')));
  t('no security text was introduced in dashboard.html',
    !/\b(GRANT|REVOKE|CREATE POLICY|ALTER TABLE)\b/.test(read('dashboard.html')));
